...
### 知识清单
- [x] AI 应用分层架构（接入层 / 编排层 / 模型层 / 数据层）
- [x] Prompt 管理与版本控制
- [x] 输出结构化（JSON Mode, Structured Output, Zod Schema）
- [x] 安全防护（Prompt Injection 检测, 内容审核, PII 过滤）
- [x] 成本优化（模型路由, 缓存策略, Prompt 压缩）
- [x] 可观测性（LangSmith / Langfuse 追踪, Token 统计）
- [x] 异步任务与队列（长时间推理, 批处理）
- [x] 多租户隔离与限流
- [x] A/B 测试与模型评估
- [x] 边缘场景处理（幻觉检测, 拒答策略, 降级方案）
...
| 2026-05-15 | 阶段5 | AI 应用架构模式知识清单 + 架构总览 | 11 篇笔记，生产级关注点 |
